...
{
	int x,y,move;
	do{cout<<"輸入地圖大小:";cin>>n;}while(n<=0); 
...
{                                         /*可改成其他圖案*/ 
	...
	while(move==7)
	{
		...
		else if(m==8){for(i=0;i<3;i++){if(y<n-1)xy[x-1+i][y+1]='=';if(x-1+i==xg-1 && y+1==yg-1)R=2;}break;}
		else if(m==6){for(i=0;i<3;i++){if(x<n-1 && y<n+1-i && y+i-1>=0)xy[x+1][y-1+i]='=';if(x+1==xg-1 && y-1+i==yg-1)R=2;}break;}
		else if(m==4){for(i=0;i<3;i++){if(x>0 && y<n+1-i && y+i-1>=0)xy[x-1][y-1+i]='=';if(x-1==xg-1 && y-1+i==yg-1)R=2;}break;}
		else if(m==2){for(i=0;i<3;i++){if(y>0)xy[x-1+i][y-1]='=';if(x-1+i==xg-1 && y-1==yg-1)R=2;}break;}
	...
	while(move==9)
	{
		...
		cout<<"4=向左\t6=向右\n5=取消"<<endl;cin>>m;
		if(m==5){cout<<"取消施放技能"<<endl;break;}
		else if(m==8){for(i=1;i<n-y;i++){xy[x][y+i]='=';if(x==xg-1 && y+i==yg-1)R=2;}break;}
		else if(m==6){for(i=1;i<n-x;i++){xy[x+i][y]='=';if(x+i==xg-1 && y==yg-1)R=2;}break;}
		else if(m==4){for(i=1;i<=x;i++){xy[x-i][y]='=';if(x-i==xg-1 && y==yg-1)R=2;}break;}
		else if(m==2){for(i=1;i<=y;i++){xy[x][y-i]='=';if(x==xg-1 && y-i==yg-1)R=2;}break;}
		else{cout<<"again"<<endl;cin>>m;}
	}
	for(j=n-1;j>=0;j--){for(i=0;i<n;i++)cout<<xy[i][j]<<" ";cout<<endl;}
	...
	else if(xg-1==x && yg-1==y){cout<<"你死了"<<endl;sc=999;}
}
void system_massage()
{
	cout<<"7=橫砍\t9=射擊\n8=向上\t2=向下"<<endl;
