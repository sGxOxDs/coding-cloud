...
namespace Photon.Chat.Demo
{
    ...
    public class ChatGui : MonoBehaviour, IChatClientListener
    {
        ...
        public void MsgDeal()
        {
            ...
            string temp = this.InputFieldChat.text;
            ...
            for(int i=0;i< PhotonNetwork.CurrentRoom.PlayerCount;i++)
            {
                ...
                {
                    this.InputFieldChat.text = "\\msg " + players[i].NickName + "," + temp;
                    return;
                }
            }
            return;
        ...
        int currentchannelmsgcount = 0;
        void ChannelMSGDeal(ChatChannel channel)
        {
            List<object> msgs = channel.Messages;
            List<string> senders = channel.Senders;

            Debug.Log("ChannelMSGDeal");

            for (int i = currentchannelmsgcount; i < msgs.Count; i++) 
            {
                if(senders[i]==PhotonNetwork.NickName)
                {
                    CurrentChannelText.text += "\t\t" + msgs[i].ToString() + "\n";
                }
                else
                {
                    CurrentChannelText.text += "" + msgs[i].ToString() + "\t\t\n";
            ...
            currentchannelmsgcount = msgs.Count;
        }
