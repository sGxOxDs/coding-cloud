...
public class Launcher : MonoBehaviourPunCallbacks
{
    ...
    private void Update()
    {
        /*if (PhotonNetwork.CurrentRoom != null && !getchatdata && PhotonNetwork.CurrentRoom.PlayerCount == PhotonNetwork.CurrentRoom.MaxPlayers)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    getchatdata = true;
                    channelname = ChatGui.instance.chatClient.GetPrivateChannelNameByUser(players[i].NickName);
                }
            }
        }
        else if (PhotonNetwork.CurrentRoom != null && PhotonNetwork.CurrentRoom.PlayerCount != PhotonNetwork.CurrentRoom.MaxPlayers)
        {
            getchatdata = false;
            chating = false;
        }
        else if (getchatdata && chating)
        {
           
            ChatGui.instance.ShowChannel(channelname);
        }*/
    ...
    public void LeaveRoom()
    {
        ...
        getchatdata = false;
        ChatGui.instance.chatClient.Disconnect();
        MenuManager.instance.OpenMenu("loading");
    }
    ...
    public void Send()
    {
        ...
        {
            ChatGui.instance.InputFieldChat.text = "";
            return;
        }
        chating = true;
        Player[] players = PhotonNetwork.PlayerList;
        ...
        for (int i=0;i<2;i++)
        {
            if(players[i] != PhotonNetwork.LocalPlayer)
            {
                string temp = ChatGui.instance.InputFieldChat.text;
                ChatGui.instance.InputFieldChat.text = "\\msg " +players[i].NickName + "," + temp;
            }
        }
    }
}
