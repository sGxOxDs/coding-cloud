...
namespace Photon.Chat.Demo
{
    ...
    public class ChatGui : MonoBehaviour, IChatClientListener
    {
        ...
        public void OnDestroy()
        {
            ...
            {
                Debug.Log("Disconnecting as: " + this.UserName);
        ...
        public void OnSubscribed(string[] channels, bool[] results)
        {
            ...
            {
                //this.chatClient.PublishMessage(channel, "join"); // you don't HAVE to send a msg on join but you could.
        ...
        public void ChannelMSGDeal(ChatChannel channel)
        {
            List<object> origin = channel.Messages;
            List<string> sender = channel.Senders;
            Debug.Log(currentmsgcount + "/" + origin + "/" + sender);
            for (int i = currentmsgcount; i < origin.Count; i++)
            {
                if (sender[i] == UserName) 
                {
                    this.CurrentChannelText.text += "\t\t" + origin[i].ToString() + '\n';
                }
                else
                {
                    this.CurrentChannelText.text += origin[i].ToString() + "\t\t\n";
