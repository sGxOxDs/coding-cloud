...
void* cutter(void* data) {
	My_Data* my_data = (My_Data*)data; // 取得輸入資料
	...
	while (true)
	{
		...
		if (my_data->have_pork)
		{
			if (my_data->process == 0 && !my_data->active)
			{
				...
				output("CUTTER#" + to_string(my_data->id), "方法1 cutting... cutting... Pork#"+ to_string(my_data->pork_deal_with.id), to_string(my_data->process));
			}
			if (my_data->process == 0)
			{
				...
				{
					my_data->process = rand() % 21 * 10 + 100;
					output("CUTTER#" + to_string(my_data->id), "方法2 cutting... cutting... Pork#" + to_string(my_data->pork_deal_with.id), to_string(my_data->process));
				}
	...
	while (true)
	{
		...
		if (my_data->have_pork)
		{
			if (my_data->process <= 0 && !my_data->active)
			{
				...
				output("PACKER#" + to_string(my_data->id), "方法1 processing & Packing the Pork#" + to_string(my_data->pork_deal_with.id), to_string(my_data->process));
			}
			if (my_data->process <= 0)
			{
				replace_pork.acquire();
				...
				{
					my_data->process = rand() % 51 * 10 + 500;
					output("PACKER#" + to_string(my_data->id), "方法2 processing & Packing the Pork#" + to_string(my_data->pork_deal_with.id), to_string(my_data->process));
				}
				else
				{
					my_data->active = false;
		...
		if (!slot.empty() && pork_break())
		{
			while (pork_break())
			{
				...
				for (int i = 0; i < slot.size(); i++)
				{
					...
					{
						temp.stage = 3;
						output("Pork#" + to_string(temp.id), "spoiled(壞掉)");
					}
				}
				replace_pork.release();
			}
