...
//      return ( ( myData.off / 4 ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
       if (myData.mapSize > 0)
       {
           ...
           MapPtr tempmap = new pointer[newmapsize]();
           for (size_type i = myData.myOff; i < myData.myOff + myData.mySize; i++)
           {
               if (tempmap[i / 4] == nullptr)
                   tempmap[i / 4] = new value_type[4]();
               tempmap[i / 4][i % 4] = myData.map[(i % (4 * myData.mapSize)) / 4][i % 4];
           }
           clear();
           delete[] myData.map;
           myData.map = tempmap;
