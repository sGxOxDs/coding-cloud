...
HugeInteger HugeInteger::operator*( HugeInteger &op2 )
{
   ...
   int* ans = new int [productSize]();
   int ansSize = productSize;

   for (int i = 0; i < multiplierSize; i++)
       for (int j = 0; j < multiplicandSize; j++)
           ans[i + j] += (it1[i] - '0') * (it2[j] - '0');
   ...
   for (int i = ansSize - 1; i >= 0; i--)
       toString.push_back(ans[i] + '0');
   product.convert(toString);
...
HugeInteger HugeInteger::operator/( HugeInteger &op2 )
{
   ...
   else
   {
       ...
       string::iterator it4 = buffer.integer.begin();
