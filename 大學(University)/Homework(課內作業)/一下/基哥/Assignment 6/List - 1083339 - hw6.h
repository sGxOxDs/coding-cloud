...
public:
      ...
      : myData()
   {
       ...
       myData.myHead->prev = myData.myHead->next = myData.myHead;
       if (right.size() > 0)
           for (iterator i = right.myData.myHead->next; i != right.myData.myHead; i = i->next)
               push_back( i->myVal );
   }
   ...
   list& operator=( const list &right )
   {
      ...
      {
          resize(right.size());
          for (iterator i = this->myData.myHead->next, j = right.myData.myHead->next;
              j != right.myData.myHead;
              i = i->next, j = j->next)
              i->myVal = j->myVal;
      }
...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
    ...
    else
    {
        ...
        for (; itl != left.end() && itr != right.end(); itl = itl->next, itr = itr->next)
            if (itl->myVal != itr->myVal)
                return false;
        return true;
    }
}
