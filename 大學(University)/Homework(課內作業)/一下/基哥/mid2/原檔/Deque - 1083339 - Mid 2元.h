   ...
   void insert( const_iterator where, const value_type &val )
   {
	   ...
	   else
	   {
		   if (begin() <= where && where <= end())
		   {
			   ...
			   {
				   if (myData.myOff % 4 == 0 && myData.mySize >= 4 * (myData.mapSize - 1))
					   doubleMapSize();
				   myData.myOff--;
				   myData.myOff %= 4 * myData.mapSize;

				   size_type i = myData.myOff % (4 * myData.mapSize);
				   size_type j = (myData.myOff + 1) % (4 * myData.mapSize);
				   size_type end = myData.mySize / 2;
				   size_type check = 0;

				   for (; check < end; check++, i++, j++)
				   {
				   ...
				   cout << "in if j:" << myData.map[(j % (4 * myData.mapSize)) / 4][(j % (4 * myData.mapSize)) % 4] << endl;
			   }
			   else  // all elements after (and at) where move backward
			   {
				   ...
				   size_type j = (myData.myOff + myData.mySize -1) % (4 * myData.mapSize);
				   size_type end = myData.mySize / 2;
				   size_type check = 0;
			       ...
			       cout << "in el j:" << myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] << endl;
			   }
		   }
	   }
	   ++myData.mySize;
	   cout << myData.mySize-1 << " : ";
	   for (auto d2 = begin(); d2 != end(); d2++)
		   cout << *d2 << " ";
	   cout << endl;
   }
   ...
   void erase( const_iterator where )
   {
	   if (myData.mySize > 0 && begin() <= where && where < end())
	   {
		   ...
		   else
		   {
			   ...
			   if (where < begin() + myData.mySize / 2)
			   {
				   size_type i = myData.myOff, j = myData.myOff + 1;
				   size_type end = (myData.myOff + (myData.mySize / 2)) % (4 * myData.mapSize);
				   for (; j < 4 * myData.mapSize && j < end; i++, j++)
				   {
			   ...
			   {
				   size_type i = myData.myOff + myData.mySize, j = myData.myOff + myData.mySize - 1;
				   size_type end = (myData.myOff + (myData.mySize / 2) + 1) % (4 * myData.mapSize);
				   for (; j > 0 && j > end; i--, j--)
				   {
					   myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];
				   }
				   if (j == 0)
				   {
					   ...
					   i = 0;
					   j = 4 * myData.mapSize - 1;
					   myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];

					   for (i = 4 * myData.mapSize - 1, j = 4 * myData.mapSize - 2; j > end; i--, j--)
					   {
						   myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];
					   }
				   }
