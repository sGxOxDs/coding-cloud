...
public:
   ...
   void insert( const_iterator where, const value_type &val )
   {
       ...
       if (myData.mySize == 0)
       {
           if (begin() == where) // added
           { // added
               myData.map = new pointer[8](); // added
               myData.map[7] = new value_type[4](); // added
               myData.map[7][3] = val; // added
               myData.mapSize = 8; // added
               myData.myOff--; // added
               myData.myOff %= 4 * myData.mapSize; // added
           } // added
           else // added
           { // added
       ...
       else
       {
           if (begin() <= where && where <= end())
           {
               ...
               {
                   if ((myData.myOff - 1) % 4 == 3 && // modified
                       myData.mySize >= 4 * (myData.mapSize - 1)) // modified
                   {
                       doubleMapSize();
                       myData.myOff %= 4 * myData.mapSize; // added
                   }
				   ...
				   size_type j = myData.myOff % (4 * myData.mapSize); // modified
				   size_type end = where.myOff % (4 * myData.mapSize); // modified
				   size_type check = 0;

				   for (; (i + 1) % (4 * myData.mapSize) != end;) // modified
				   {
					   if (myData.map[(i % (4 * myData.mapSize)) / 4] == nullptr)
						   myData.map[(i % (4 * myData.mapSize)) / 4] = new value_type[4]();
                       j++; // added
					   myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] = myData.map[(j % (4 * myData.mapSize)) / 4][(j % (4 * myData.mapSize)) % 4];
                       i++; // added
				   }
                   if (myData.map[(i % (4 * myData.mapSize)) / 4] == nullptr) // added
                       myData.map[(i % (4 * myData.mapSize)) / 4] = new value_type[4](); // added
				   myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] = val;
				   // cout << "in if j:" << myData.map[(j % (4 * myData.mapSize)) / 4][(j % (4 * myData.mapSize)) % 4] << endl;  deleted
			   }
			   else  // all elements after (and at) where move backward
			   {
				   if ((myData.myOff + myData.mySize) % 4 == 0 && myData.mySize >= 4 * (myData.mapSize - 1))
					   doubleMapSize();
				   myData.myOff %= 4 * myData.mapSize;

				   size_type i = (myData.myOff + myData.mySize) % (4 * myData.mapSize);
				   size_type j = (myData.myOff + myData.mySize) % (4 * myData.mapSize); // modified
				   size_type end = where.myOff % (4 * myData.mapSize); // modified
				   size_type check = 0;

				   for (; (i) % (4 * myData.mapSize) != end;) // modified
				   {
					   if (myData.map[(i % (4 * myData.mapSize)) / 4] == nullptr)
						   myData.map[(i % (4 * myData.mapSize)) / 4] = new value_type[4]();
                       if (i % (4 * myData.mapSize) == end) // added
                           break; // added
                       j--; // added
					   myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] = myData.map[(j % (4 * myData.mapSize)) / 4][(j % (4 * myData.mapSize)) % 4];
                       i--; // added
                   ...
                   if (myData.map[(i % (4 * myData.mapSize)) / 4] == nullptr) // added
                       myData.map[(i % (4 * myData.mapSize)) / 4] = new value_type[4](); // added
				   myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] = val;
				   //cout << "in el i:" << myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] << endl;
			       //cout << "in el j:" << myData.map[(i % (4 * myData.mapSize)) / 4][(i % (4 * myData.mapSize)) % 4] << endl;  deleted
			   }
		   }
	   }
	   ++myData.mySize;
	   //cout << myData.mySize-1 << " : ";  deleted
	   //for (auto d2 = begin(); d2 != end(); d2++)  deleted
		   //cout << *d2 << " ";  deleted
	   //cout << endl;  deleted
   }
   ...
   void erase( const_iterator where )
   {
	   if (myData.mySize > 0 && begin() <= where && where < end())
	   {
		   ...
		   else
		   {
			   ...
			   if (where < begin() + myData.mySize / 2)
			   {
				   size_type i = where.myOff % (4 * myData.mapSize), j = where.myOff % (4 * myData.mapSize); // modified
				   size_type end = (myData.myOff) % (4 * myData.mapSize); // modified
				   for (;i % (4 * myData.mapSize) != end;) // modified
				   {
                       (--i) %= (4 * myData.mapSize); // added
					   myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];
                       (--j) %= (4 * myData.mapSize); // added

				   }
				   //if (j == 4 * myData.mapSize)  deleted
				   //{  deleted
					   //j = 0;  deleted
					   //myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];  deleted

					   //for (i = 0, j = 1; j < end; i++, j++)  deleted
					   //{  deleted
						   //myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];  deleted
					   //}  deleted
				   //}  deleted
				   myData.myOff++;
				   //myData.myOff %= 4 * myData.mapSize;  deleted
			   }
			   else // all elements after where move forward
			   {
                   size_type i = where.myOff % (4 * myData.mapSize), j = where.myOff % (4 * myData.mapSize); // modified
                   size_type end = (myData.myOff + myData.mySize) % (4 * myData.mapSize); // modified
				   for (; i % (4 * myData.mapSize) != end;) // modified
				   {
                       (++i) %= (4 * myData.mapSize); // added
                       if (i % (4 * myData.mapSize) == end) // added
				   ...
				   //{  deleted
					   ...
					   //i = 0;  deleted
					   //j = 4 * myData.mapSize - 1;  deleted
					   //myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];  deleted

					   //for (i = 4 * myData.mapSize - 1, j = 4 * myData.mapSize - 2; j > end; i--, j--)  deleted
					   //{  deleted
						   //myData.map[j / 4][j % 4] = myData.map[i / 4][i % 4];  deleted
					   //}  deleted
				   //}  deleted
